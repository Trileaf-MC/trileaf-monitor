...
public class PanelHandlerFactory {
    private static final Map<String, PanelHandler> registry = new ConcurrentHashMap<>();
    ...
    public PanelHandlerFactory(Map<String, PanelHandler> handlerMap) {
        registry.clear();
        handlerMap.values().forEach(handler -> registry.put(handler.getPanelType(), handler));
    }
    ...
    public static PanelHandler getPanelHandler(String panelType) {
        ...
        PanelHandler handler = registry.get(panelType);
